...
export default function ProductPage() {
  ...
  const handleCreateProduct = () => {
    axios.post('http://localhost:3002/api/produits', newProduct)
      .then(response => {
        fetchProducts();
        setNewProduct({
          nomProd: '',
  ...
  const handleDeleteProduct = productId => {
    axios.delete(`http://localhost:3002/api/produits/${productId}`)
      .then(response => {
        fetchProducts();
      })
      .catch(error => {
  ...
  {products.map(product => {
    ...
    return (
      ...
      <Card>
        ...
        />
        <CardContent>
        <Typography variant="body2">Prix: {product.prixProd} Dt</Typography>
  ...
  <div>
    ...
    if (selectedProductId === product._id) {
      ...
      color: '#ffffff', marginBottom:"1px",}}>{selectedProductId === product._id ? 'Masquer' : 'Afficher '}</Button>
                  <Button
                  onClick={() => handleEditProduct(product)}
                  sx={{ backgroundColor: '#007bff', color: '#ffffff',marginBottom:"5px" }}
                >
                  Modifier
                </Button>
                <Button
                  onClick={() => handleDeleteProduct(product._id)}
                  sx={{ backgroundColor: '#dc3545', color: '#ffffff' ,marginBottom:"5px",marginRight:'19px'}}
                >
                  Supprimer
                </Button>
          </CardActions>
